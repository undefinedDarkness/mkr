...
guint get_number_of_children(GtkWidget *flowbox) {
	GList *children = gtk_container_get_children(flowbox);
	guint count = g_list_length(children);
	g_list_free(children); // Free the list (not the widgets!)
	return count;
}


void recalc_items(APP_MUT) {
	// printf("== SYS == Recalc\n");
	app->userdata.visibleItems = get_number_of_children(app->ui.display);
	gtk_flow_box_invalidate_sort(app->ui.display);
	gtk_flow_box_invalidate_filter(app->ui.display);

	bool isAnyVisible = app->userdata.visibleItems != 0;
	if (!isAnyVisible) {
...
// NOTE: Maybe this function could be gotten from a loaded library to allow
// custom libraries.
void layout_rofi(GtkWidget *window, APP_MUT) {
   ...
   * */
	...
	gtk_box_pack_start(header, input, false, false, 0);
	ADD(layout, header);
	...
	gtk_scrolled_window_set_min_content_height(scroll, 0);
