...
GtkWidget *font_preview(Result * selectedResult)
{
	...
	gtk_label_set_markup(alphabet,
			     g_strdup_printf
			     ("<span font=\"%s\">Whereas disregard and contempt for human rights have resulted </span>",
			      pango_font_family_get_name(family)));
...
void font_generate(API)
{
	...
	for (int i = 0; i < len; i++) {
		...
		AUTO fname = gtk_label_new("");
		CLASS(fname, "font-family-name");
		ADD(box, fname);
		gtk_label_set_markup(fname,
				     g_strdup_printf
				     ("<span font=\"%s\">%s</span>", r->label,
				      r->label));
		gtk_label_set_xalign(fname, 0);
		AUTO row = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
		gtk_box_pack_end(box, row, false, false, 0);

		if (pango_font_family_is_variable(family)) {
			AUTO variable = gtk_label_new("VARIABLE");
			CLASS(variable, "bubble");
			ADD(row, variable);
		}
		if (pango_font_family_is_monospace(family)) {
			AUTO monospace = gtk_label_new("MONOSPACE");
			CLASS(monospace, "bubble");
			ADD(row, monospace);
		}
