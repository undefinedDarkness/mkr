			  ...
			  DLState * state)
{
	...
	state->api.update_progress(state->completed / state->fsize,
				   state->api.data);
...
void dl_execute(const char *url, API)
{
	...
	AUTO ofile =
	    g_file_new_build_filename(g_get_user_special_dir
	...
	state->api = *api;
	AUTO finfo =
	    g_file_input_stream_query_info(fstream,
					   G_FILE_ATTRIBUTE_STANDARD_SIZE, NULL,
					   NULL);
	// g_print("File Size:%lu\n ", g_file_info_get_size(finfo));
	state->fsize = g_file_info_get_size(finfo);
	state->completed = 0;
	state->ofstream =
	    g_file_append_to(ofile, G_FILE_CREATE_NONE, NULL, NULL);
