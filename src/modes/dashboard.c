...
static GtkWidget* launcherBtn(char *iconName, char*execute) {
	...
	AUTO btn = gtk_button_new_from_icon_name(iconName, GTK_ICON_SIZE_LARGE_TOOLBAR);
...
GtkWidget* dashboard(void*) {
	...
	AUTO launcher = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);CLASS(launcher,"dashboard-launcher");
	...
	AUTO quote = gtk_label_new("<span font=\"Liberation Serif\"><i>Nothing is impossible,The word \nitself says 'I'm possible'</i></span>");
