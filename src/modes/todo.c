...
void todo_generate(API) {
	...
	char* line = NULL; int len; GList* list = NULL;
	while (g_io_channel_read_line(io, &line, &len, NULL, NULL) != G_IO_STATUS_EOF) {
		char**parts = g_strsplit(line, "\t", 2);
		AUTO wid = gtk_check_button_new_with_label(g_strstrip(parts[1]));
