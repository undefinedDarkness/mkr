...
static void modeItems(APP_MUT)
{
	...
	if (app->currentMode.metadata.type & HAS_PREVIEW) {
		g_signal_connect(display, "row-activated", display_preview,
				 app);
	}
...
static void modeItemCleanup(Mode prevMode, APP)
{
	...
	while (children != NULL) {
		...
		AUTO res = g_object_get_data(child, "__resptr");
		prevMode.clean(res);
...
{
	g_print("Changing mode from %s to %s\n", prevMode.label,
		app->currentMode.label);


	gtk_container_remove(app->ui.preview, gtk_bin_get_child(app->ui.preview));
